...
const AuthenticateSpotify = props => {
  ...
  const authenticateWithSpotify = async () => {
    ...
    if (error) {
      ...
      // Start the auth flow.
      const spotifyRedirectFunctionURL = 'https://us-central1-music-queue-dev.cloudfunctions.net/spotifyRedirect';
      ...
      const spotifyTokenFunctionURL = `https://us-central1-music-queue-dev.cloudfunctions.net/spotifyToken?code=${encodeURIComponent(code)}&state=${encodeURIComponent(state)}`;
  ...
  if (error) {
    return (
      <div>
        {`Error back from the Spotify auth page: ${error.message}`}
      </div>
    );
  }
